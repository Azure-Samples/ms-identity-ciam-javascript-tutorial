...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (ciam-msal-react-spa)"
   # create the application 
   $clientAadApplication = New-MgApplication -DisplayName "ciam-msal-react-spa" `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/ciam-msal-react-spa")
    ...
    Write-Host "Done creating the client application (ciam-msal-react-spa)"
    ...
    Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>ciam-msal-react-spa</a></td></tr>" -Path createdApps.html
    ...
    # print the registered app portal URL for any further navigation
    Write-Host "Successfully registered and configured that app registration for 'ciam-msal-react-spa' at `n $clientPortalUrl" -ForegroundColor Green 
