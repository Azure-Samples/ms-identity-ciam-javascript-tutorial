...
Function ConfigureApplications
{
   ...
   Write-Host "Creating the AAD application (ciam-msal-node-cli-app)"
   # create the application 
   $clientAadApplication = New-MgApplication -DisplayName "ciam-msal-node-cli-app" `
                                                      -PublicClient `
                                                      @{ `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/ciam-msal-node-cli-app")
    ...
    Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>ciam-msal-node-cli-app</a></td></tr>" -Path createdApps.html
    ...
    Write-Host "Successfully registered and configured that app registration for 'ciam-msal-node-cli-app' at `n $clientPortalUrl" -ForegroundColor Green 
